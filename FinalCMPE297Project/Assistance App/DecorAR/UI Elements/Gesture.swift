...
class Gesture {
	...
	let virtualObject: VirtualModels
	
	var refreshTimer: Timer?
	
	init(_ touches: Set<UITouch>, _ sceneView: ARSCNView, _ virtualObject: VirtualModels) {
		currentTouches = touches
		self.sceneView = sceneView
		self.virtualObject = virtualObject
		
		// Refresh the current gesture at 60 Hz - This ensures smooth updates even when no new touch events are incoming (but the camera might have moved).
		self.refreshTimer = Timer.scheduledTimer(withTimeInterval: 0.016_667, repeats: true, block: { _ in
			self.refreshCurrentGesture()
		})
	}
	
	static func startGestureFromTouches(_ touches: Set<UITouch>, _ sceneView: ARSCNView, _ virtualObject: VirtualModels) -> Gesture? {
		if touches.count == 1 {
...
class SingleFingerGesture: Gesture {
	...
	override init(_ touches: Set<UITouch>, _ sceneView: ARSCNView, _ virtualObject: VirtualModels) {
		...
		for result in results {
			if VirtualModels.isNodePartOfVirtualObject(result.node) {
	...
	func finishGesture() {
		...
		// the object.
		if !objectHit || approxScreenSpaceCoveredByTheObject() > 0.5 { //0.5
