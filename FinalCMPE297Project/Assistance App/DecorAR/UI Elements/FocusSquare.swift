...
class FocusSquare: SCNNode {
    ...
    private let focusSquareColor = #colorLiteral(red: 0.2392156869, green: 0.6745098233, blue: 0.9686274529, alpha: 1)
	private let focusSquareColorLight = #colorLiteral(red: 0.2588235438, green: 0.7568627596, blue: 0.9686274529, alpha: 1)
	...
	func hide() {
		if self.opacity == 1.0 {
			self.runAction(SCNAction.fadeOut(duration: 0.1))
		}
	}
	
	func unhide() {
		if self.opacity == 0.0 {
			self.runAction(SCNAction.fadeIn(duration: 0.1))
		}
	}
    ...
    /* This function reduces size changes of the focus square based on the distance by scaling it up if it far away,
      and down if it is very close.The values are adjusted such that scale will be 1 in 0.7 m distance (estimated distance when looking at a table),
